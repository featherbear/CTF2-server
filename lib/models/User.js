...
class User {
  ...
  static async login (username, password) {
    const userData = await User.getUser(username, true)
  ...
  static async getUser (username, asJSON = false) {
    return asJSON ? await database.getUser(username) : new User(username)
  }
  ...
  async getName () {
    return (await User.getUser(this.username, true)).name
  }
