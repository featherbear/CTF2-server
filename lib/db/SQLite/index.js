...
export default class {
  ...
  getUserIdFromUsername (username) {
    return this.db.prepare(`SELECT id FROM ${ENUM.Users} WHERE username = ?`).get(username).id
  }
  ...
  getScores () {
    return this.db.prepare(`SELECT name, SUM(scores) AS score FROM ${ENUM.Solves} GROUP BY id INNER JOIN SELECT name FROM ${ENUM.Users} WHERE ${ENUM.Users}.id = ${ENUM.Solves}.user`)
  }

  getSolves () {
    return this.db.prepare(`SELECT * FROM ${ENUM.Solves}`)
    // return this.db.prepare(`SELECT question, COUNT(user) AS solves FROM ${ENUM.Solves} GROUP BY question`)
  }
