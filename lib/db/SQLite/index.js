...
export default class {
  ...
  renameCategory ({ id, name }, newName) {
    let query
    if (typeof id !== 'undefined') {
      query = this.db.prepare(`UPDATE ${ENUM.Users} SET name = @name WHERE id = @targetId`)
    } else {
      query = this.db.prepare(`UPDATE ${ENUM.Users} SET name = @name WHERE name = @targetName`)
    }
  ...
  deleteCategory ({ id, name }) {
    let query
    if (typeof id !== 'undefined') {
      query = this.db.prepare(`DELETE FROM ${ENUM.Users} WHERE id = @targetId`)
    } else {
      query = this.db.prepare(`DELETE FROM ${ENUM.Users} name = @targetName`)
    }
