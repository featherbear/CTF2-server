...
export default class {
  ...
  renameCategory (id, newName) {
    const query = this.db.prepare(`UPDATE ${ENUM.Users} SET name = @name WHERE id = @id`)
  ...
  deleteCategory (id) {
    return this.db.prepare(`DELETE FROM ${ENUM.Users} WHERE id = ?`).run(id)
  }
  /* QUESTIONS */
  ...
  updateQuestion (questionId, title, description, category, flag, value) {

  }

  deleteQuestion (questionId) {
    this.db.prepare(`DELETE FROM ${ENUM.Users} WHERE id = ?`).run(questionId)
